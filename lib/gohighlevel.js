...
export async function findOrCreateContact(contactInput) {
    ...
    console.log('findOrCreateContact: Starting upsert:', {
    ...
    const upsertPayload = buildContactPayload({
        ...contactInput,
        email: normalizedEmail,
        locationId: LOCATION_ID
    });

    console.log('findOrCreateContact: Upsert payload:', {
        email: upsertPayload.email,
        firstName: upsertPayload.firstName,
        lastName: upsertPayload.lastName,
        phone: upsertPayload.phone,
        tags: upsertPayload.tags,
        locationId: upsertPayload.locationId
    });

    try {
        const upsertResponse = await ghlRequest('/contacts/upsert', {
            method: 'POST',
            body: upsertPayload
        });

        console.log('findOrCreateContact: Upsert response received:', {
            hasResponse: !!upsertResponse,
            responseKeys: upsertResponse ? Object.keys(upsertResponse) : [],
            hasContact: !!upsertResponse?.contact,
            hasContactsArray: Array.isArray(upsertResponse?.contacts),
            contactId: upsertResponse?.contact?.id || upsertResponse?.id,
            responsePreview: JSON.stringify(upsertResponse).substring(0, 500)
        });

        const extractedContact = extractContactFromResponse(upsertResponse);
        
        console.log('findOrCreateContact: Extracted contact:', {
            hasContact: !!extractedContact,
            contactId: extractedContact?.id,
            contactEmail: extractedContact?.email,
            contactKeys: extractedContact ? Object.keys(extractedContact) : []
        });

        if (!extractedContact || !extractedContact.id) {
        ...
        console.error('findOrCreateContact: Error during upsert:', {
            email: normalizedEmail,
...
export async function removeTagsFromContact(contactId, tags) {
    ensureApiKey('removeTagsFromContact');
    ...
    // GoHighLevel API uses DELETE method to remove tags
    return ghlRequest(`/contacts/${contactId}/tags/`, {
        method: 'DELETE',
        body: {
            tags
