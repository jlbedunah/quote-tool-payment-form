...
function buildAuthorizeNetConfigPriority() {
    const isProductionEnv = process.env.NODE_ENV === 'production' || process.env.AUTHORIZE_NET_ENVIRONMENT === 'production';

    const productionConfig = {
        environment: 'production',
        endpoint: 'https://api.authorize.net/xml/v1/request.api',
        loginId: process.env.AUTHORIZE_NET_LOGIN_ID_PROD,
        transactionKey: process.env.AUTHORIZE_NET_TRANSACTION_KEY_PROD
    };

    const sandboxConfig = {
        environment: 'sandbox',
        endpoint: 'https://apitest.authorize.net/xml/v1/request.api',
        loginId: process.env.AUTHORIZE_NET_LOGIN_ID,
        transactionKey: process.env.AUTHORIZE_NET_TRANSACTION_KEY
    };

    return isProductionEnv ? [productionConfig, sandboxConfig] : [sandboxConfig, productionConfig];
}
