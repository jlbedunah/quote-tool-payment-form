...
async function updateQuotePaymentStatus(normalized) {
    if (!supabase) {
        console.log('updateQuotePaymentStatus: Missing supabase');
        return null;
    }

    // Need either quoteId or email to find the quote
    if (!normalized.quoteId && !normalized.email) {
        console.log('updateQuotePaymentStatus: Missing both quoteId and email');
        return null;
    }
    ...
    try {
        ...
        // If quoteId is provided, use it directly (most reliable)
        if (normalized.quoteId) {
            console.log('updateQuotePaymentStatus: Looking up quote by ID:', normalized.quoteId);
            ...
            if (findError) {
                console.error('Error finding quote by ID:', findError);
                // Fall through to email-based lookup
            } else if (quoteData) {
                quote = quoteData;
                console.log('updateQuotePaymentStatus: Found quote by ID:', {
                    id: quote.id,
        ...
        // Fall back to email-based lookup if quoteId didn't work
        if (!quote && normalized.email) {
            ...
            if (findError) {
                console.error('Error finding quote by email:', findError);
                return null;
            }
            ...
            const matchingQuotes = (allPendingQuotes || []).filter(q =>
                q.customer_email &&
                q.customer_email.toLowerCase().trim() === emailToMatch
            );

            if (matchingQuotes.length > 0) {
                quote = matchingQuotes[0];
                console.log('updateQuotePaymentStatus: Found quote by email:', {
                    id: quote.id,
                    email: quote.customer_email,
                    status: quote.payment_status
                });
        ...
        if (!quote) {
            console.log('No quote found for:', {
...
function normalizeAuthorizeNetPayload(eventBody) {
    ...
    return {
        ...
        rawPayload: payload,
        // Custom field for quote tracking (passed from direct payment, not webhooks)
        quoteId: eventBody.quoteId || null
    };
}
...
Created: ${new Date().toLocaleString()}`;
    ...
    // Check if this is a payment plan subscription (name starts with "Payment Plan -")
    const isPaymentPlan = subscriptionName.startsWith('Payment Plan -') ||
                          subscriptionName.startsWith('PP-') ||
                          (subscription.order?.invoiceNumber && subscription.order.invoiceNumber.startsWith('PP-'));

    let allTags;
    if (isPaymentPlan) {
        // For payment plans, use payment-plan-specific tags
        allTags = ['payment-plan-active'];
        console.log('Detected payment plan subscription - using payment plan tags');
    } else {
        // Get product-specific tags based on interval for regular subscriptions
        const productTags = getSubscriptionTagsByInterval(interval);

        // Combine generic and product-specific tags
        allTags = ['subscription-created', ...productTags];
    }
...
Cancelled: ${new Date().toLocaleString()}`;
    ...
    // Check if this is a payment plan subscription
    const isPaymentPlan = subscriptionName.startsWith('Payment Plan -') ||
                          subscriptionName.startsWith('PP-') ||
                          (subscription.order?.invoiceNumber && subscription.order.invoiceNumber.startsWith('PP-'));

    // Remove active subscription tags based on type
    const tagsToRemove = [];
    const tagToAdd = isPaymentPlan ? 'payment-plan-cancelled' : 'subscription-cancelled';

    if (isPaymentPlan) {
        // For payment plans, remove payment-plan-active tag
        tagsToRemove.push('payment-plan-active');
        console.log('Detected payment plan cancellation - using payment plan tags');
    } else {
        // For regular subscriptions, remove interval-based tags
        if (interval) {
    ...
    // Add cancellation tag (different for payment plans)
    try {
        await addTagsToContact(contactId, [tagToAdd]);
