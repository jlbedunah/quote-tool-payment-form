...
import { notifySaleMade } from './slack-notifications.js';
...
export async function syncAuthorizeNetTransaction(eventBody) {
    ...
    try {
        ...
        if (slackResult.success) {
            ...
            console.error('❌ Slack notification failed:', {
                ...
                fullResult: slackResult
            });
        }
    } catch (slackError) {
        console.error('❌ Error sending Slack notification (non-fatal):', {
    ...
    try {
        ...
        console.error('Error sending GA4 purchase event (non-fatal):', ga4Error);
...
export async function syncAuthorizeNetSubscription(eventBody) {
    ...
    if (!email) {
        console.warn('Authorize.net subscription webhook missing customer email. Skipping CRM sync.', {
            subscriptionId,
            subscriptionName
        });
